...
5. 通过每次执行副作用函数时重新执行一次依赖收集，解决了分支切换遗留副作用函数的问题。
6. 引入 effect 栈以支持 effect 函数的嵌套调用。
7. 通过在每次触发更新前判断移除当前正激活的副作用函数，解决了无限递归循环问题。
8. 为 effect 函数新增 options 参数，并通过其 scheduler 属性实现了副作用函数的调度执行。后续的 computed 和 watch 正式基于此功能才实现的。
9. 用 options 参数的 lazy 属性实现懒执行的 effect，并且能够在外部手动调用副作用函数获取到返回值。
10. 基于上一节实现的功能，我们实现了 computed，并且带有缓存功能。
...
13. 进一步实现了 watch 的两个特性：立即执行 watch 和回调的执行时机。
14. 通过为 watch 的回调函数新增 onInvalidate 参数，实现了在回调中注册清理函数以清理过期副作用的功能。