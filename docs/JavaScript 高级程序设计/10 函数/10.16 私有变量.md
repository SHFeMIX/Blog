...
这个模式是吧所有私有变量和函数都定义在构造函数中，然后创建一个能够访问这些私有成员的特权方法。在创建 MyObject 实例之后，没有办法直接访问私有变量和函数，唯一办法是通过 publicMethod。其实就是一个闭包，只是没有显式返回 publicMethod。
...
跟第一种的区别是把 publicMethod 定义在原型上，并且构造函数声明成全局变量外部也可以访问，用它新建实例。
这个模式与前一个模式的主要区别是，私有变量和函数是由实例共享。也还是闭包，publicMethod 始终引用着包含它的作用域。
